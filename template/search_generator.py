...
def create_sql_agent_for_db(db, db_type, memory):
    ...
    prompt_template = PromptTemplate(
        ...
        ## Oracle Instructions:
        - Use `FETCH FIRST 100 ROWS ONLY` to limit rows (Oracle 12c+).
        - For older versions, use `WHERE ROWNUM <= 100`.
        - **NEVER USE** `LIMIT` in Oracle.
        ...
        Q: Tampilkan semua pajak rumah  
        A:  
        SELECT * FROM pajak_rumah FETCH FIRST 100 ROWS ONLY;
        
        Q: Tampilkan guru kominfo
        A:
        SELECT TOP 100 * FROM guru_kominfo;
        ---
